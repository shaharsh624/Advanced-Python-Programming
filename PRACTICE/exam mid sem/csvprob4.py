...
with open(r'D:\Fifth Sem\Advance Python\Practical - Theory\exam mid sem\product.csv','r') as csvfile:
    reader=csv.DictReader(csvfile)
    for row in reader:
...
for product_id, total_quantity_sold in productsales.items():
    ...
    summary_data.append([product_id, product_name, total_quantity_sold, average_quantity_sold])
...
with open('sales_summary.csv','w',newline='') as csvfile:
    ...
    writer.writerow(['Product ID', 'Product Name', 'Total Quantity Sold', 'Average Quantity Sold per Month'])
